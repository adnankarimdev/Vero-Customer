...
export default function DuolingoPlatform({
  ...
  const progress = (allSelectedBadges.length / categories.reduce((acc, cat) => acc + cat.badges.length, 0)) * 100;
    ...
    <Card className="w-full max-w-2xl border-0 bg-[#235390] text-white shadow-lg rounded-3xl overflow-hidden">
      <CardHeader className="bg-[#58CC02] p-4 flex flex-row items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <CardTitle className="text-xl font-bold">{businessName} Review</CardTitle>
        </div>
        <div className="flex items-center space-x-2">
      ...
      <CardContent className="p-6">
        ...
        <Progress value={progress} className="mb-6 bg-[#1C4477] h-3" indicatorClassName="bg-[#58CC02]" />
        {isLoading ? (
            ...
            {[...Array(3)].map((_, index) => (
              <div key={index} className="animate-pulse">
                ...
                <div className="flex flex-wrap gap-2">
                  {[...Array(4)].map((_, i) => (
                    <div key={i} className="h-10 w-24 bg-[#1C4477] rounded-xl"></div>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="space-y-6">
            {categories.map((category, categoryIndex) => (
              <div key={category.name} className="bg-[#1C4477] p-4 rounded-xl">
                ...
                <p className="mb-3 text-white/80">Select the correct options:</p>
      ...
      <CardFooter className="flex justify-between items-center bg-[#1C4477] p-4">
        <Button variant="ghost" className="text-white hover:text-[#58CC02]" onClick={stopTimer}>
        ...
        <AlertDialog open={isAlertDialogOpen}>
          <AlertDialogContent className="bg-[#235390] text-white">
            <AlertDialogHeader>
              <AlertDialogTitle className="text-2xl font-bold">Great job! 🎉</AlertDialogTitle>
            ...
            <AlertDialogFooter>
              <AlertDialogCancel onClick={handleSaveReviewWithoutGenerate} className="bg-[#1C4477] text-white hover:bg-[#1C4477]/80">
                No, thanks
              </AlertDialogCancel>
              <AlertDialogAction onClick={handleGenerateReview} className="bg-[#58CC02] text-white hover:bg-[#58CC02]/90">
                Yes, share progress!
              </AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
