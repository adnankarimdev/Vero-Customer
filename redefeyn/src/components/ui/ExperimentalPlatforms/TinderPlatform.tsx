...
const defaultCategories = [
  {
    ...
    badges: ["Friendly staff", "Quick service", "Attentive", "Knowledgeable", "Accommodating"],
  },
  {
    name: "Ambiance and Environment",
    badges: ["Cozy atmosphere", "Great view", "Convenient parking", "Quiet area", "Outdoor seating"],
  },
  {
    name: "Quality of Items",
    badges: ["Rich flavor", "Perfect temperature", "Great variety", "Ethically sourced", "Unique blends"],
  },
]
...
export default function TinderFiveStarReviewBuilder({
  ...
  const handleSaveReview = async (duration: number) => {
    ...
    try {
      await axios.post("https://vero.ngrok.dev/backend/save-customer-review/", { data: dataToSave })
      if (inStoreMode) {
  ...
  const handleLike = () => {
    ...
    setSelectedBadges(prev => ({
      ...prev,
      [currentCategory.name]: [...(prev[currentCategory.name] || []), currentBadge]
    }))
    nextBadge()
  }
  ...
  const nextBadge = () => {
    if (currentBadgeIndex < categories[currentCategoryIndex].badges.length - 1) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <Card className="w-full max-w-md border-0 shadow-lg">
        <CardHeader>
          <CardTitle className="text-center text-2xl font-bold">{businessName}</CardTitle>
          <CardDescription className="text-center mb-2">
            {rating <= 4 ? "Want to tell us why?" : "We are so happy to hear that. 🥳 Want to tell us why?"}
          </CardDescription>
          <div className="flex items-center justify-center space-x-1">
            {[...Array(5)].map((_, i) => (
              <Star
                key={i}
                className={cn("w-5 h-5", i < rating ? "text-yellow-400 fill-yellow-400" : "text-gray-300")}
              />
            ))}
          </div>
        </CardHeader>
        <CardContent>
          <h3 className="text-lg font-semibold mb-4 text-center">{currentCategory.name}</h3>
          <motion.div
            drag="x"
          ...
          <div className="flex justify-between mt-4">
            <Button variant="outline" onClick={handleDislike}>👎 Dislike</Button>
            <Button variant="outline" onClick={handleLike}>👍 Like</Button>
          </div>
        </CardContent>
        <CardFooter className="flex justify-center">
          <Button variant="ghost" onClick={stopTimer} disabled={Object.keys(selectedBadges).every(key => selectedBadges[key].length === 0)}>
            <Send className="mr-2" />
            Submit
          </Button>
        </CardFooter>
