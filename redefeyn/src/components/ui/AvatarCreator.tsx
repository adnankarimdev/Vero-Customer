...
const Eyes = ({ style }: any) => {
  const eyeMap = {
    ...
    oval: (
      <>
        <ellipse cx="35" cy="45" rx="6" ry="4" />
        <ellipse cx="65" cy="45" rx="6" ry="4" />
      </>
    ),
    square: (
      <>
    ...
    almond: (
      <>
        <path d="M30 45 Q35 35 40 45 Q35 55 30 45" />
        <path d="M60 45 Q65 35 70 45 Q65 55 60 45" />
      </>
    ),
    wink: (
      <>
        <circle cx="35" cy="45" r="5" />
        <path d="M60 45 Q65 40 70 45" />
      </>
    ),
    lashes: (
      <>
    ...
    sunglasses: (
      <>
        <path
          d="M25 45 L45 45 Q50 45 55 45 L75 45"
          fill="none"
          stroke="black"
          strokeWidth="3"
        />
...
const Mouth = ({ style }: any) => {
  const mouthMap = {
    ...
    line: (
      <line x1="30" y1="70" x2="70" y2="70" stroke="#000000" strokeWidth="3" />
    ),
    open: <ellipse cx="50" cy="70" rx="15" ry="10" />,
    smirk: (
      <path
        d="M30 70 Q50 75 70 65"
        fill="none"
        stroke="#000000"
        strokeWidth="3"
      />
...
export default function AvatarCreator() {
  ...
  const handleSaveAvatar = async () => {
    const svgComponent = (
      <AvatarSVG color={color} eyeStyle={eyeStyle} mouthStyle={mouthStyle} />
    );
    const svgString = ReactDOMServer.renderToString(svgComponent);
    console.log(svgString); // Outputs SVG as string
    axios
      .post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/backend/save-user-avatar/`,
        { email: localEmail, svg: svgString },
      )
      .then((response) => {
        toast({
          title: "Saved",
    ...
    //axios post svg & email, usetoast for save succesful or fail
  };
